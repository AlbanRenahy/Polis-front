...
const polisApiMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case CONNECT_USER:
      ...
      axios
        ...
        .then((response) => {
          ...
          store.dispatch(
            updateFormField("firstNameInput", response.data.firstname)
          );
          store.dispatch(
            updateFormField("lastNameInput", response.data.lastname)
          );
          ...
          store.dispatch(
            updateFormField("loginMessage", "Vous êtes connecté(e)")
          );
      ...
      axios
        ...
        .catch((error) => {
          ...
          store.dispatch(
            updateFormField(
              "loginMessage",
              "Veuillez vous connecter pour contribuer à Polis."
            )
          );
          store.dispatch(updateFormField("isConnected", false));
          setTimeout(
            () =>
              store.dispatch(updateFormField("loginStatus", "not-connected")),
            1500
          );
        });
      break;
    ...
    case FORGOTTEN_PASSWORD:
      axios
        .post(`${polisApi}/forgottenPassword`, {
          email: store.getState().username,
        })
        .then((response) => {
        ...
        .post(
          `${polisApi}/createBuilding`,
          {
            ...
            style: store.getState().architectureInput
              ? store.getState().architectureInput
              : null,
            ...
            creationDate: store.getState().dateInput
              ...
              : null,
            surface: store.getState().surfaceInput
              ? parseInt(store.getState().surfaceInput)
              : null,
            architect: store.getState().architectInput
              ? store.getState().architectInput
              : null,
            promoter: store.getState().promoterInput
              ? store.getState().promoterInput
              : null,
            builder: store.getState().builderInput
              ? store.getState().builderInput
              : null,
            planner: store.getState().plannerInput
              ? store.getState().plannerInput
              : null,
            urbanist: store.getState().urbanistInput
              ? store.getState().urbanistInput
              : null,
            ...
            certified: false,
            delivered:
              store.getState().dateInput === ""
                ? true
                : parseInt(store.getState().dateInput) < date.getFullYear(),
          },
          {
            withCredentials: true,
          }
    ...
    case GET_BUILDINGS:
      ...
      axios
        .post(`${polisApi}/buildings/screen`, { bounds: action.bounds })
        .then((response) => {
    ...
    case GET_BUILDINGS_LIST_DATA:
      next(action);
      const queries = action.list.map((item) =>
        axios.get(`${polisApi}/buildings/${item.id}`)
      );
      axios
        .all(queries)
        .then((results) => {
